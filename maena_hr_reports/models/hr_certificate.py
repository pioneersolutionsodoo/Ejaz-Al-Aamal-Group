...
class HrCertificate(models.Model):
    ...
    def onchange_employee_id(self):
        self.date_start = self.employee_id.aj_date

    @api.model
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            vals['name'] = self.env['ir.sequence'].next_by_code('hr.certificate') or _('New')
        return super(HrCertificate, self).create(vals)

    def unlink(self):
        for rec in self:
            if rec.state != 'draft':
                raise UserError(_('You cannot delete record which is not in Draft.'))
        return super(HrCertificate, self).unlink()
