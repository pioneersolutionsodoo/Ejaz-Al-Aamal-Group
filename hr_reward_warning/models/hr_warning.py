...
class HrAnnouncementTable(models.Model):
    ...
    def send_by_email(self):
        self.ensure_one()
        request_template = self.env.ref('hr_reward_warning.mail_template_announcement', raise_if_not_found=False)
        for emp in self.employee_ids:
            email = emp.work_email if emp.work_email else emp.address_home_id
            if not email:
                raise ValidationError(_("Please enter email for Employee or Employee Partner!"))
            email_values = {
                'email_to': emp.work_email,
                'email_from': self.env.user.email_formatted,
            }
            if request_template:
                request_template.send_mail(self.id, email_values=email_values)