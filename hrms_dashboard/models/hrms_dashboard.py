...
class hr_employee_loan_ps_model(models.Model):
    _inherit = 'hr.employee.loan.ps'


class hr_performance_evaluation_model(models.Model):
    _inherit = 'hr.performance.evaluation'


class hr_business_trip_model(models.Model):
...
class Employee(models.Model):
    ...
    birthday = fields.Date('Date of Birth', groups="hr.group_hr_user", help="Birthday")
    ...
    #         else:
    #             age = False
    #         if employee[0]['aj_date']:
    #             diff = relativedelta(datetime.today(), employee[0]['aj_date'])
    #             years = diff.years
    #             months = diff.months
    #             days = diff.days
    ...
    def get_user_employee_details(self):
        ...
        query = """
            ...
            from hr_leave
            WHERE (hr_leave.date_from::DATE,hr_leave.date_to::DATE) OVERLAPS ('%s', '%s') and 
            employee_id='%s'""" % (today, today, employee_id.id)
        cr = self._cr
        cr.execute(query)
        leaves_today = cr.fetchall()
        print('1::', query)
        ...
        query = """
                    ...
                    from hr_leave
                    WHERE (hr_leave.date_from::DATE,hr_leave.date_to::DATE) OVERLAPS ('%s', '%s')
                    and employee_id='%s'""" % (first_day, last_day, employee_id.id)
        cr = self._cr
        cr.execute(query)
        leaves_this_month = cr.fetchall()
        employee_memos = self.env['employee.memo'].search([('employee_id', '=', employee_id.id)])
        employee_memos_req = len(employee_memos)
        performance_ref = self.env['hr.performance.evaluation'].search([('employee_id', '=', employee_id.id)])
        performance_ref_req = len(performance_ref)
        expenses = self.env['hr.expense'].search([('employee_id', '=', employee_id.id)])
        expenses_req = len(expenses)
        timesheets = self.env['account.analytic.line'].sudo().search([('employee_id', '=', employee_id.id)])
        timesheet_count = len(timesheets)

        payslips = self.env['hr.payslip'].sudo().search([('employee_id', '=', employee_id.id)])
        payslips_count = len(payslips)
        print(payslips_count)
        Overtimes = self.env['hr.overtime.ps'].search([('employee_id', '=', employee_id.id)])
        All_Overtimes = len(Overtimes)
        Vacation = self.env['vacation.resettlement'].search([('employee_id', '=', employee_id.id)])
        All_Vacation = len(Vacation)
        Loan = self.env['hr.employee.loan.ps'].search([('employee_id', '=', employee_id.id)])
        All_loan = len(Loan)
        Advance = self.env['hr.employee.advance.ps'].search([('employee_id', '=', employee_id.id)])
        All_advance = len(Advance)
        air_tiket = self.env['air.ticket'].search([('employee_id', '=', employee_id.id)])
        All_air_tiket = len(air_tiket)
        bus_trip = self.env['hr.business.trip'].search([('employee_id', '=', employee_id.id)])
        All_bus_trip = len(bus_trip)
        mobile_phone = employee_id.user_id.work_phone
        department = employee_id.department_id.name

        # if self.env.user.has_group('')
        timesheet_view_id = self.env.ref('hr_timesheet.hr_timesheet_line_search')
        Leaves_view_id = self.env.ref('pioneer_HR_Leave.action_hr_holidays_saudi_summary')
        # job_applications = self.env['hr.applicant'].sudo().search_count([])
        if employee:
            sql = """select broad_factor from hr_employee_broad_factor where id =%s"""
            self.env.cr.execute(sql, (employee[0]['id'],))
            ...
            if employee:
                data = {
                    'broad_factor': broad_factor if broad_factor else 0,
                    'leaves_to_approve': All_Leaves,
                    'overtime_request': All_Overtimes,
                    'vacation_request': All_Vacation,
                    'loan_request': All_loan,
                    'advance_request': All_advance,
                    'at_request': All_air_tiket,
                    'bt_request': All_bus_trip,
                    'performance_ref_req': performance_ref_req,
                    'expenses_req': expenses_req,
                    'mobile_phone': mobile_phone,
                    'department': department,
                    'leaves_today': leaves_today,
                    'leaves_this_month': leaves_this_month,
                    'leaves_alloc_req': employee_memos_req,
                    'emp_timesheets': timesheet_count,
                    'timesheet_view_id': timesheet_view_id,
                    'Leaves_view_id': Leaves_view_id,
                    'experience': experience,
                    'age': age,
                    'hr_payslip': payslips_count,
                }
                employee[0].update(data)
            return employee
        else:
    ...
    def get_upcoming(self):
        ...
        event = cr.fetchall()
        announcement = []
        if employee and (self.env.user.has_group('hr.group_hr_user') or self.env.user.has_group('hr.group_hr_manager')):
            department = False
            job_id = False
...
group by hr_employee.department_id,hr_department.name""")
    ...
    def join_resign_trends(self):
        ...
        cr.execute('''select to_char(aj_date, 'Month YYYY') as l_month, count(id) from hr_employee 
        WHERE aj_date BETWEEN CURRENT_DATE - INTERVAL '12 months'
    ...
    def get_attrition_rate(self):
        ...
        for month_date in month_start_list:
            ...
            where aj_date < date '%s'
            """ % (month_date[0], month_date[0],))
